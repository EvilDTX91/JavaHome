package lottomadness;
import java.awt.*;
import javax.swing.*;
public class LottoMadness extends JFrame {
    
    LottoEvent lotto = new LottoEvent(this);
    
    //az 1. sor létrehozása
    JPanel row1 = new JPanel();
    ButtonGroup option = new ButtonGroup();
    JCheckBox quickpick = new JCheckBox("Quick Pick", false);
    JCheckBox personal = new JCheckBox("Personal", true);
    
    //a 2. sor létrehozása
    JPanel row2 = new JPanel();
    JLabel numbersLabel = new JLabel("Your picks: ", JLabel.RIGHT);
    //JTextField[] numbers = new JTextField[6];
    JTextField[] numbers = new JTextField[5]; // házi
    JLabel winnersLabel = new JLabel("Winners: ", JLabel.RIGHT);
    //JTextField[] winners = new JTextField[6];
    JTextField[] winners = new JTextField[5]; // házi
    
    //a 2.5 sor létrehozása házifeladat a soroslás sebességének állíthatósága
    JPanel row25 = new JPanel();
    JSlider speedSlider = new JSlider(0,300);
    //JLabel speedLabel = new JLabel("Speed:", JLabel.RIGHT);
    //JTextField speed = new JTextField("100");
    
    //a 3. sor létrehozása
    JPanel row3 = new JPanel();
    JButton stop = new JButton("Stop");
    JButton play = new JButton("Play");
    JButton reset = new JButton("Reset");
    
    //a 4. sor létrehozása
    JPanel row4 = new JPanel();
    JLabel got3Label = new JLabel("3 of 6: ", JLabel.RIGHT);
    JTextField got3 = new JTextField("0");
    JLabel got4Label = new JLabel("4 of 6: ", JLabel.RIGHT);
    JTextField got4 = new JTextField("0");
    JLabel got5Label = new JLabel("5 of 6: ", JLabel.RIGHT);
    JTextField got5 = new JTextField("0");
    JLabel got6Label = new JLabel("6 of 6: ", JLabel.RIGHT);
    JTextField got6 = new JTextField("0", 10);
    JLabel drawingsLabel = new JLabel("Drawings: ", JLabel.RIGHT);
    JTextField drawings = new JTextField("0");
    JLabel yearsLabel = new JLabel("Years: ", JLabel.RIGHT);
    JTextField years = new JTextField();
    
    //a 5. sor létrehozása
    JPanel row5 = new JPanel();
    JLabel author = new JLabel("Készítette: ", JLabel.RIGHT);
    JTextField aut = new JTextField("Nádas Tamás");
    
    public LottoMadness()
    {
        super("Lotto Madness");
        setSize(550, 400);
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        GridLayout layout = new GridLayout(6, 1, 10, 10);
        setLayout(layout);
        
        //figyelők hozzá adása
        //ezek reagálnak a felhasználói beavatkozásra
        quickpick.addItemListener(lotto);
        personal.addItemListener(lotto);
        stop.addActionListener(lotto);
        play.addActionListener(lotto);
        reset.addActionListener(lotto);
        //+++++++++++++
      
        //első sor rendezése
        FlowLayout layout1 = new FlowLayout(FlowLayout.CENTER, 10, 10);
        /*'CENTER' a komponensek középre igazítottak ('10' szélessége, '10' magassága)
        azaz 10 képpontnyi a tárolóknak az egymástól való távolsága*/
        option.add(quickpick); //hozzá adja gomb csoporthoz
        option.add(personal);
        row1.setLayout(layout1);
        row1.add(quickpick);//hozzá adja a panelhez
        row1.add(personal);
        add(row1);
        
        //második sor rendezése
        GridLayout layout2 = new GridLayout(2, 7, 10, 10);
        row2.setLayout(layout2);
        row2.add(numbersLabel);
        //for(int i = 0; i < 6; i++)
        for(int i = 0; i < 5; i++) //házi    
        {
            numbers[i] = new JTextField();
            row2.add(numbers[i]);
        }
        row2.add(winnersLabel);
        //for(int i = 0; i < 6; i++)
        for(int i = 0; i < 5; i++) // házi
        {
            winners[i] = new JTextField();
            winners[i].setEditable(false);
            row2.add(winners[i]);
        }
        add(row2);
        
        //2.5 sor rendezése házifeladat a soroslás sebességének állíthatósága
        GridLayout layout25 = new GridLayout(1, 1, 20, 20);
        row25.setLayout(layout25);
        speedSlider.setMajorTickSpacing(20);
        speedSlider.setMinorTickSpacing(10);
        speedSlider.setPaintTicks(true);
        speedSlider.setPaintLabels(true);
        row25.add(speedSlider);
        //row25.add(speedLabel);
        //row25.add(speed);
        add(row25);
        
        //hamradik sor rendezése
        FlowLayout layout3 = new FlowLayout(FlowLayout.CENTER, 10, 10);
        row3.setLayout(layout3);
        stop.setEnabled(false);
        row3.add(stop);
        row3.add(play);
        row3.add(reset);
        add(row3);
        
        //negyedik sor rendezése
        GridLayout layout4 = new GridLayout(2, 3, 20, 10);
        row4.setLayout(layout4);
        row4.add(got3Label);
        got3.setEditable(false);
        row4.add(got3);
        row4.add(got4Label);
        got4.setEditable(false);
        row4.add(got4);
        row4.add(got5Label);
        got5.setEditable(false);
        row4.add(got5);
        row4.add(got6Label);
        got6.setEditable(false);
        row4.add(got6);
        row4.add(drawingsLabel);
        drawings.setEditable(false);
        row4.add(drawings);
        row4.add(yearsLabel);
        years.setEditable(false);
        row4.add(years);
        add(row4);
        
        // ötödik sor rendezése 
        GridLayout layout5 = new GridLayout(1,2,50,10);
        row5.setLayout(layout5);
        row5.add(author);
        row5.add(aut);
        aut.setEditable(false);
        add(row5);
        
        setVisible(true);
    }
    
    public static void main(String[] args) {
        LottoMadness frame = new LottoMadness();
    }
    
}
